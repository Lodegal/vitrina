...
jQuery(function ($) {
  ...
  function initFlagsSlider() {
    var $SliderElement = $('.js-flags-slider'),
        $SliderShow = $SliderElement.find('.slideshow'),
        $SliderNav = $SliderElement.find('.flags-slider__nav'),
        $SliderDotsArrowPrev = $SliderNav.find('.flags-slider__arrow_prev'),
        $SliderDotsArrowNext = $SliderNav.find('.flags-slider__arrow_next');
    ...
    if ($SliderElement.length) {
      ...
      $SliderShow.slick({
        ...
        $(".flag-icon[data-toggle='tooltip']").tooltip('hide');
      });
    }
  } // ADVANTAGE TABS
  ...
  function initAdvantageTabs() {
    if ($('#advantage').length) {
      ...
      $('.advantage-tabs__link[data-toggle="tab"]').on('shown.bs.tab', function (e) {
        ...
        $targetTab == "owner" ? $substrate.removeClass('-right-side ') : $substrate.addClass('-right-side');
      });
    }
  }
});